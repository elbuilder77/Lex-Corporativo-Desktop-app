                      ...
                      return (
                        <motion.article key={result.id || index} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0"><span className={cn('inline-flex rounded-lg border px-2 py-1 text-xs font-bold', module === 'mercantil' ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700')}>{result.article_number || result.subtitle || 'Disposición'}</span><h3 className="mt-2 truncate text-sm font-bold text-slate-950">{result.law_code || result.title || 'Fuente normativa'}</h3></div>
