    ...
    <div className="flex h-full flex-col bg-slate-50 text-slate-900">
      <header className="shrink-0 border-b border-slate-200 bg-white px-5 py-3 md:px-8">
        <div className="flex min-h-12 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-fiscal">Lex Fiscal</p>
      ...
      <section className="hidden shrink-0 items-center gap-4 border-b border-slate-200 bg-slate-50 px-5 py-2.5 md:flex md:px-8" aria-label="Avance de la operación fiscal">
        <div className="min-w-0 w-52 shrink-0">
          ...
          <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">{completedCount} de {OPERATION_STEPS.length} etapas completas</p>
        </div>
        <div className="grid min-w-0 flex-1 grid-cols-4 gap-1">
          {OPERATION_STEPS.map((item) => {
            ...
            return (
              <button key={item.step} type="button" onClick={() => setActiveTab(item.tab)} className={cn('inline-flex min-w-0 items-center justify-center gap-1 rounded-lg px-1 py-2 text-[10px] font-bold transition', active ? 'bg-fiscal text-white' : 'text-slate-600 hover:bg-white', completed && !active && 'text-emerald-700')}>
                <CheckCircle2 size={13} className={cn('shrink-0', completed ? 'opacity-100' : 'opacity-35')} /> <span className="truncate">{item.label}</span>
              </button>
            );
          })}
        </div>
        <button type="button" onClick={() => navigate('/portafolio')} className="inline-flex min-w-max items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-bold text-slate-700 hover:bg-slate-100"><FolderOpen size={14} /> Portafolio</button>
      </section>
